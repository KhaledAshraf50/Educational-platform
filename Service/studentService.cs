...
//        {
//            if (id <= 0)
//                throw new ArgumentException("رقم الطالب غير صحيح", nameof(id));
...
using Luno_platform.Viewmodel;
using Microsoft.EntityFrameworkCore;
namespace Luno_platform.Service { public class studentService : BaseService<Student>, IstudentService { private IstudentRepo _repository;
        public studentService(IstudentRepo repository) : base(repository) { _repository = repository; } 
        public Student GetStudent(int id) { return _repository.GetStudent(id); } 
        public List<Courses> GetStudentCourses(int id) { return _repository.GetStudentCourses(id); }
        public List<StudentCourseFullDataVM> GetStudentCoursesFullData(int id) { return _repository.GetStudentCoursesFullData(id); }
        public List<Courses> GetStudentCourses(int studentId, int page = 1, int pageSize = 10) { return _repository.GetStudentCourses(studentId, page, pageSize);}
        public List<Payments> GetPayments(int studentId) { return _repository.GetPayments(studentId); }
    } }